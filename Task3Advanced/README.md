...
Дополнение к целевой архитектуре Task 3: явное распределение ответственностей текущего ESB-хаба.

## Принцип

ESB централизует валидацию, трансформации, маршрутизацию, надёжность и интеграционную логику в одном «умном центре». Целевая архитектура следует обратному принципу: событийная шина становится простым транспортом, а логика расщепляется и уезжает в домены и платформенные слои. Переносить эту логику в один новый сервис нельзя — это повторило бы исходную проблему централизации.

## Соответствие функций ESB → новый дом

| Функция ESB (Camel)            | Новый дом                   | Механизм                                                                                                                                      |
|--------------------------------|-----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| ФЛК / форматная валидация      | Реестр схем + контракты     | Проверка по зарегистрированной схеме на публикации события; только обратносовместимые версии схем                                             |
| Логическая / бизнес-валидация  | Доменные сервисы (агрегаты) | Инварианты охраняются на границе агрегата самим владельцем данных (DDD)                                                                       |
| Потоковая трансформация        | Потоковая обработка (Flink) | Stream-джобы на событиях, near-real-time витрины; потребитель сам преобразует событие в свою модель                                           |
| Пакетная трансформация         | Lakehouse / дата-продукты   | Доменные batch-конвейеры (dbt, medallion Bronze→Silver→Gold); CDC из легаси                                                                   |
| Маршрутизация                  | Событийная шина (Kafka)     | Декларативный publish/subscribe — топики + consumer groups вместо императивного роутера; content-based — через дизайн топиков / Kafka Streams |
| Паттерны надёжности            | Kafka + прикладные паттерны | Durability, репликация, replay в Kafka; идемпотентные потребители, DLQ, outbox, retry с backoff, саги, circuit breaker в сервисах             |
| Интеграционная / бизнес-логика | Домены + оркестрация (саги) | Бизнес-логика — в доменах; межсервисные процессы — хореография через события или сага в выделенном контексте                                  |

## Связь с остальными артефактами

- **Надёжность** напрямую опирается на митигации из карты рисков (риск A3: идемпотентность, DLQ, outbox) — надёжная доставка ESB не теряется, а уже заложена в плане управления рисками.
- **Валидация и бизнес-логика** ложатся на агрегаты из Task 4 — каждый контекст валидирует и обрабатывает свои данные сам.
- На время миграции часть этих функций ещё выполняет ESB через ACL (Strangler Fig); по мере перевода доменов на события они переезжают в новые дома, после чего ESB выводится.
